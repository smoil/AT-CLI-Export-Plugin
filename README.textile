...
Example with multiple options: <code>atcli.command mydatabasepassword ead ~/path/for/exported/files/ -fas Completed InProcess -rid1 someid orsomeotherid oreventhisid -io</code>

Note: params with multiple words are not yet supported but should be soon.  Currently each word would be interpreted as a separate param
